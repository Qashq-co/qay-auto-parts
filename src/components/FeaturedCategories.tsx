...
export const FeaturedCategories: React.FC<FeaturedCategoriesProps> = ({ setCurrentPage, setSelectedCategory }) => {
  return (
    <section className="py-16 sm:py-24 bg-white relative overflow-hidden" id="categories">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 sm:space-y-16 relative z-10">
        ...
        <div className="text-center space-y-2.5 sm:space-y-3">
          <p className="text-brand-orange font-extrabold tracking-widest text-[9px] sm:text-[11px] uppercase bg-orange-500/5 inline-block px-3 py-1 rounded border border-brand-orange/15 shadow-sm max-w-full truncate">
            Engine Rebuild & Replacement Components
          </p>
          <h2 className="text-2xl sm:text-4xl lg:text-5xl font-black text-brand-navy tracking-tight uppercase">
            FEATURED <span className="text-brand-orange">ENGINE PARTS</span>
          </h2>
          <div className="w-12 sm:w-16 h-1 bg-brand-orange mx-auto rounded-full mt-2 shadow-sm"></div>
        </div>
 
        {/* Responsive Grid Layout - Clean 2 columns on small screens scaling to 5 columns on desktop */}
